...
#include <Adafruit_BME280.h> //Contiene i comandi specifici per il chip BME280
extern Telemetry current_data;

Adafruit_BME280 bme;  // indica che ho creato un oggetto virtuale che rappresenta il chip fisico (l'ho capito così)

bool init_BME280(){
    if (! bme.begin(0x76)) {
        Serial.println("ERRORE: BME280 non trovato!"); // Messaggio per il team 
        return(false); }
